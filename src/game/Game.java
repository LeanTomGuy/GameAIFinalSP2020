...
public class Game implements Cloneable {
	...
	public Game(Game game) {
		this.agentTurn = game.agentTurn;
		this.turn = game.turn;
		this.whoseTurn = game.whoseTurn;
		this.board = game.board.copyBoard();
		this.moveLst = game.moveLst;
		this.wInCheck = game.wInCheck;
		this.bInCheck = game.bInCheck;
		this.gameOver = game.gameOver;
		// cloned.sc = this.sc;
		this.wKingPos = game.wKingPos;
		this.bKingPos = game.bKingPos;
		this.turnLimit = game.turnLimit;
	}
	...
	public int getScore(PlayerType player) {
		int score = 0;
		for (Piece pce : retPceLst(player)) {
			score += pce.getScore();
		}
	...
	public List<Piece> retPceLst(PlayerType player) {
...
//		System.out.println(move.toString());
		...
		if (!(move.getPiece().isValidMove(move.getFinalPosition(), board.copySquares(), this.clone()))
			...
			// System.out.println(move.getFinalPosition().toString());
			 System.out.println("err; Not valid, try again.");
			// System.out.println("check " + !(move.getInitPosition().equals(move.getPiece().getPosition())));
...
//				System.out.println("new:" + whoseTurn);
	...
	public Board simulateMove(Move move) {
		...
		if (move.isCastle()) {
			...
			movedPiece.setPosition(move.getFinalPosition());
			/*
			 * System.out.println(move.toString()); System.out.println("(sim) initial at " +
			 * move.getInitPosition().toString() + "|" + move.getInitPosition().getRank() +
			 * move.getInitPosition().getFile()); System.out.println("final at " +
			 * move.getFinalPosition().toString() + "|" + move.getFinalPosition().getRank()
			 * + move.getFinalPosition().getFile());
			 */
	...
	public void advanceGameSim(Move move) {
		// System.out.println(move.toString());
		Piece movedPiece = Piece.copyPiece(move.getPiece());
		if (move.isCastle()) {
...
//			System.out.println("not castle");
			if (movedPiece.toString().equals("R") || movedPiece.toString().equals("K")) { // movedPiece.hasMoved = true;
				cantCastle = true;
			}
			if (move.isCapture()) {
