...
// normal moves
		{
			if (this.getPlayer() == PlayerType.W) {
				return (thisRank < fin.getRank() && squares[fin.getRank()][fin.getFile()].getPiece().getPlayer() == PlayerType.NONE);
			} else {
				return (thisRank > fin.getRank() && squares[fin.getRank()][fin.getFile()].getPiece().getPlayer() == PlayerType.NONE);
			}
		}
...
//This is for normal pawn moves and first pawn moves.
		...
		for (int cnt = 0; cnt < pathLength; cnt++) {
			path[cnt] = new Position(this.getPosition().getFile(), Math.min(this.getPosition().getRank(), finalPos.getRank()) + cnt);
		}
